...
Current implementation note, May 2026: this document is a design reference.
The active macOS build path is the Swift package script
\filepath{swift/fileprovider/build.sh}, release packaging produces a
\code{.pkg} via \filepath{scripts/macos-build-pkg.sh}, and notarization is
attempted but non-blocking in release CI. The PZM evidence is non-production
Mac App Development/testing-mode proof, not production Developer ID clean-host
Finder acceptance.
...
\begin{enumerate}
  ...
  \item \textbf{On-demand hydration} --- files exist as APFS dataless files
...
\midrule
Linux   & FUSE3                & tcfs-fuse         & Kernel FUSE module           & Host-proven lifecycle \\
Windows & Cloud Files API      & tcfs-cloudfilter  & CFAPI minifilter             & Skeleton \\
macOS   & FileProvider         & tcfs-fileprovider & APFS dataless files          & PZM testing-mode lab proof \\
iOS     & FileProvider         & tcfs-fileprovider & APFS dataless files          & Proof-of-concept \\
\bottomrule
\end{tabularx}
\caption{Per-platform file integration strategy}
\end{table}

All four platforms share the same hydration concept: a lightweight remote-file
representation is visible before content is local; content is fetched on
demand from SeaweedFS via the CAS chunk store.
...
# 3. Build and sign Swift host app + FileProvider extension
swift/fileprovider/build.sh

# 4. Assemble the macOS installer package
scripts/macos-build-pkg.sh \
    --cli-tar tcfs-<version>-macos-aarch64.tar.gz \
    --fileprovider-zip TCFSProvider.zip \
    --out tcfs-<version>-macos-aarch64.pkg

# 5. Notarize when credentials are available
swift/fileprovider/notarize.sh TCFSProvider.zip
\end{lstlisting}
...
\begin{enumerate}
  ...
  \item Swift package build for the host app and extension bundle
  \item Code signing with explicit certificate/profile gates
  \item Optional notarization via \code{notarytool}
  \item Archive as FileProvider zip and macOS \code{.pkg} release assets
\end{enumerate}
...
\midrule
Pre-hydration representation & Clean VFS entry, or \code{.tc} key/value file in physical sync roots & CFAPI placeholder & APFS dataless file \\
