...
- `task lazy:home-canary-linux-xr-shadow` now creates the next real-project
  ...
  85 skipped symlinks; the next packet must run with `sync_symlinks = true` and
  prove matching symlink targets before the full parity claim is available.
