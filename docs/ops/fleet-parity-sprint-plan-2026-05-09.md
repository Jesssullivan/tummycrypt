...
| A2a. Generic git repo canary | `TIN-133`, `#309` adjacent | Run `task lazy:git-repo-canary` against one clean git worktree, defaulting to `~/git/oauth-mux`. The helper refuses dirty sources unless explicitly allowed, snapshots into `~/TCFS Pilot/real-canaries/`, and writes a packet that says no live repo, Finder, broad `~/git`, or home takeover is claimed. | Source-built small-repo parity is green in `docs/release/evidence/git-repo-canary-oauth-mux-sourcebin-fresh-20260515T014640Z/`: fresh-prefix push, 0 skipped symlinks, honey mounted traversal/hydration, 9 mounted symlink target checks, and Linux lifecycle companion. Current Nix flake packages pass the tiny symlink mounted parse/target proof in `docs/release/evidence/tcfs-symlink-package-probe-20260515T060330Z/`. Homebrew remains blocked because installed Homebrew `tcfs 0.12.12` skips symlinks. Next proof should rebuild/publish Homebrew or rerun the small-repo canary with explicit current Nix package binaries, then repeat with `~/git/linux-xr-fast` as the large clean stress canary. |
