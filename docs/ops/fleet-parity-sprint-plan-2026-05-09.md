...
| A2a. Generic git repo canary | `TIN-133`, `#309` adjacent | Run `task lazy:git-repo-canary` against one clean git worktree, defaulting to `~/git/oauth-mux`. The helper refuses dirty sources unless explicitly allowed, snapshots into `~/TCFS Pilot/real-canaries/`, and writes a packet that says no live repo, Finder, broad `~/git`, or home takeover is claimed. | Small-repo parity is green for source-built binaries in `docs/release/evidence/git-repo-canary-oauth-mux-sourcebin-fresh-20260515T014640Z/` and for explicit current Nix flake package binaries in `docs/release/evidence/git-repo-canary-oauth-mux-nixpkg-20260515T133843Z/`: fresh-prefix push, 0 skipped symlinks, honey mounted traversal/hydration, 9 mounted symlink target checks, and Linux lifecycle companion. Homebrew remains blocked because installed Homebrew `tcfs 0.12.12` skips symlinks. Next proof should rebuild/publish Homebrew if Homebrew is the client lane, then repeat with `~/git/linux-xr-fast` as the large clean stress canary and add fresh-tree restore/rollback proof. |
