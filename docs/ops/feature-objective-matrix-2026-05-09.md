...
| S3-first storage posture | Prove the S3/SeaweedFS object model, chunk sizing, retry behavior, per-chunk timeout behavior, queue/concurrency posture, and endpoint/TLS/credential class are acceptable for project-tree workloads. | Open, with mounted warning closure. `home-canary-linux-xr-shadow-20260511T040325Z/` remains functional project-tree proof, not storage posture. `home-canary-linux-xr-storage-posture-20260514T021513Z/` completed a release-binary fresh-prefix push of the 7.7 GB shadow with 92,969 upload rows / 8.23 GB / 327,482 chunks / no retry or error rows, then reused the same prefix for honey mounted `find -maxdepth 8`, exact `.clang-format` hydration, and all 85 mounted symlink target checks. The original mounted follow-up recorded 274 `NoSuchKey` warnings. A directory-prefix-only VFS rerun still recorded 274, proving that was not the full root cause. The exact `.tc` filename fix rerun passed with 0 `NoSuchKey`, 0 WARN, and 0 ERROR rows, and preserved real linux-xr ftrace `.tc` filenames in mounted traversal. The dominant 6.2 GB raw Git `.pack` remains fixed at 1,211 chunks, and the adjacent 45.6 MB `.rev` fell from 8,405 chunks to 8 chunks. The packet still does not claim production storage posture because the Linux lifecycle companion was not run in this storage packet, the endpoint was plaintext tailnet HTTP, socket highwater reached 11 while upload concurrency was 8, `.idx` still reaches 4,599 chunks, and generated AMD header files reached 2,986/2,121 chunks under the default profile. | `task lazy:home-canary-linux-xr-storage-posture`, `scripts/home-canary-linux-xr-storage-posture.sh`, `scripts/test-home-canary-linux-xr-storage-posture.sh`, `docs/release/evidence/home-canary-linux-xr-storage-posture-tc-extfix-20260514T202343Z/mounted-followup-tc-extension-fix.md`, `docs/release/evidence/home-canary-linux-xr-storage-posture-vfsfix-20260514T193104Z/mounted-followup-vfsfix.md`, `docs/release/evidence/home-canary-linux-xr-storage-posture-20260514T021513Z/storage-posture-live-observations.md`, `docs/release/evidence/home-canary-linux-xr-storage-posture-20260514T021513Z/mounted-followup.env`, `docs/release/evidence/home-canary-linux-xr-storage-posture-20260514T021513Z/push-storage-summary.env`, prior packet `docs/release/evidence/home-canary-linux-xr-storage-posture-20260513T220442Z/storage-posture-live-observations.md`, older blocker packets `docs/release/evidence/home-canary-linux-xr-storage-posture-20260513T174944Z/storage-posture-live-observations.md` and `docs/release/evidence/home-canary-linux-xr-storage-posture-20260512T034347Z/storage-posture-live-observations.md`, `docs/BENCHMARKS.md`, storage rows in `docs/ops/lazy-traversal-qa-permutation-matrix-2026-05-09.md`, `crates/tcfs-chunks/src/fastcdc.rs`, and `crates/tcfs-sync/src/engine.rs`. | Keep the `.pack`/`.rev` profile fixes and mounted warning closure as accepted. Next, decide generated-large-file policy, reduce or justify `.idx` and generated-header chunk counts, fix socket-pool accounting, and rerun with lifecycle plus endpoint/TLS posture before upgrading the storage claim. Multipart/native SeaweedFS writes and raw `.git` defaults remain future decisions if object counts remain high. |
