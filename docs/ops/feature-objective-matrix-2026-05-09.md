...
| Generic git repo canary | Shadow one clean worktree before any live repo move, starting with `~/git/oauth-mux` and then `~/git/linux-xr-fast` for large clean stress. | Green for source-built binaries only. `git-repo-canary-oauth-mux-sourcebin-fresh-20260515T014640Z/` proves clean `oauth-mux` source/shadow inventory, source-built neo push with 0 skipped symlinks, honey mounted traversal/hydration, 9 mounted symlink target checks, and the Linux lifecycle companion. Packaged binaries remain blocked: installed Homebrew `tcfs 0.12.12` skips symlinks, and the first staged honey `tcfs 0.12.12` mounted the tree but could not parse version-3 symlink index entries. | Green packet `docs/release/evidence/git-repo-canary-oauth-mux-sourcebin-fresh-20260515T014640Z/`, plan-only packet `docs/release/evidence/git-repo-canary-oauth-mux-20260515T000411Z/`, blocker packets `docs/release/evidence/git-repo-canary-oauth-mux-20260515T003124Z/` and `docs/release/evidence/git-repo-canary-oauth-mux-20260515T003543Z/`, isolated probe `docs/release/evidence/tcfs-symlink-config-probe-20260515T005858Z/`, `task lazy:git-repo-canary`, `scripts/git-repo-canary.sh`, `scripts/test-git-repo-canary.sh`, `cargo test -p tcfs-cli cli_directory_push_preserves_symlink_when_configured`, and `docs/ops/git-repo-canary-dogfood.md`. | Rebuild/publish the symlink-preserving package lane and reproven it on both hosts before moving any active live repo into TCFS. Then repeat with `~/git/linux-xr-fast` and add rollback/fresh-tree restore proof before broad `~/git` or `~/Documents` management. Keep local symlink `sync-status`/`unsync` semantics out of the claim until they have their own contract. |
