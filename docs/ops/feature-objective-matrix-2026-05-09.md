...
| S3-first storage posture | Prove the S3/SeaweedFS object model, chunk sizing, retry behavior, per-chunk timeout behavior, queue/concurrency posture, and endpoint/TLS/credential class are acceptable for project-tree workloads. | Open. `home-canary-linux-xr-shadow-20260510T201809Z/` is useful pre-fix evidence: a large `.idx` exploded into 72,598 chunks and a 6.2 GB `.pack` showed about 6.1 GiB snapshot footprint before upload. `home-canary-linux-xr-shadow-20260511T040325Z/` is useful functional/storage-observation evidence: the `.idx` shape is improved, the 6.2 GB `.pack` still dominates object count, and mounted depth-8 traversal passed but was slow. `home-canary-linux-xr-storage-posture-20260512T034347Z/` is release-binary/fresh-prefix blocker evidence: the large pack completed, but multi-minute no-progress/no-retry gaps and slow small-file walk keep production posture open. Fresh-prefix direct manifest/index publish and the new opt-in `TCFS_UPLOAD_FILE_CONCURRENCY` canary knob reduce avoidable serial S3 work, but production storage posture still needs a fresh host packet. | `task lazy:home-canary-linux-xr-storage-posture`, `scripts/home-canary-linux-xr-storage-posture.sh`, `scripts/test-home-canary-linux-xr-storage-posture.sh`, `docs/release/evidence/home-canary-linux-xr-shadow-20260510T201809Z/storage-posture-observations.md`, `docs/release/evidence/home-canary-linux-xr-shadow-20260511T040325Z/push-storage-summary.env`, `docs/release/evidence/home-canary-linux-xr-storage-posture-20260512T034347Z/storage-posture-live-observations.md`, `docs/BENCHMARKS.md`, storage rows in `docs/ops/lazy-traversal-qa-permutation-matrix-2026-05-09.md`, `crates/tcfs-chunks/src/fastcdc.rs`, and `crates/tcfs-sync/src/engine.rs`. | Rerun the large raw-Git proof through `task lazy:home-canary-linux-xr-storage-posture` on a new disposable prefix with a timeout-enabled release binary, progress telemetry, chunk write timeout rows, fresh-prefix publish rows, endpoint/TLS/credential posture, memory, retries, object counts, `file_upload_concurrency`, and wall-clock/hydration timings; then decide whether multipart/native SeaweedFS semantics are the next product change. |
