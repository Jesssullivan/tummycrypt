...
| Generic git repo canary | Shadow one clean worktree before any live repo move, starting with `~/git/oauth-mux` and then `~/git/linux-xr-fast` for large clean stress. | Green for source-built and explicit current Nix flake package binaries, but restore/rollback is now a measured blocker. `git-repo-canary-oauth-mux-sourcebin-fresh-20260515T014640Z/` proves clean `oauth-mux` source/shadow inventory, source-built neo push with 0 skipped symlinks, honey mounted traversal/hydration, 9 mounted symlink target checks, and the Linux lifecycle companion. `git-repo-canary-oauth-mux-nixpkg-20260515T133843Z/` repeats the same proof with current Nix flake package binaries on both hosts. `git-repo-canary-oauth-mux-nixpkg-20260515T133843Z/restore-proof/` records the fresh-tree restore blocker: `tcfs reconcile` dry-run timed out after 120s while scanning the remote index, before restore execution. Homebrew remains blocked: installed Homebrew `tcfs 0.12.12` skips symlinks. | Restore blocker `docs/release/evidence/git-repo-canary-oauth-mux-nixpkg-20260515T133843Z/restore-proof/`, green packet `docs/release/evidence/git-repo-canary-oauth-mux-nixpkg-20260515T133843Z/`, source-built packet `docs/release/evidence/git-repo-canary-oauth-mux-sourcebin-fresh-20260515T014640Z/`, Nix tiny mounted packet `docs/release/evidence/tcfs-symlink-package-probe-20260515T060330Z/`, plan-only packet `docs/release/evidence/git-repo-canary-oauth-mux-20260515T000411Z/`, blocker packets `docs/release/evidence/git-repo-canary-oauth-mux-20260515T003124Z/` and `docs/release/evidence/git-repo-canary-oauth-mux-20260515T003543Z/`, isolated probe `docs/release/evidence/tcfs-symlink-config-probe-20260515T005858Z/`, `task lazy:git-repo-canary`, `task lazy:git-repo-restore-proof`, `scripts/git-repo-canary.sh`, `scripts/git-repo-restore-proof.sh`, `scripts/test-git-repo-canary.sh`, `scripts/test-git-repo-restore-proof.sh`, `cargo test -p tcfs-cli cli_directory_push_preserves_symlink_when_configured`, and `docs/ops/git-repo-canary-dogfood.md`. | Rebuild/publish the symlink-preserving Homebrew lane if Homebrew is the client lane. Fix or bypass the `reconcile` remote-index restore path, then repeat with `~/git/linux-xr-fast` and add green rollback/fresh-tree restore proof before moving any active live repo into TCFS or broadening to `~/git` / `~/Documents`. Keep local symlink `sync-status`/`unsync` semantics out of the claim until they have their own contract. |
