...
| S3-first storage posture | Prove the S3/SeaweedFS object model, chunk sizing, retry behavior, per-chunk timeout behavior, queue/concurrency posture, and endpoint/TLS/credential class are acceptable for project-tree workloads. | Open, with push-only progress. `home-canary-linux-xr-shadow-20260511T040325Z/` remains functional project-tree proof, not storage posture. `home-canary-linux-xr-storage-posture-20260514T021513Z/` completed a release-binary fresh-prefix push of the 7.7 GB shadow with 92,969 upload rows / 8.23 GB / 327,482 chunks / no retry or error rows. The dominant 6.2 GB raw Git `.pack` remains fixed at 1,211 chunks, and the adjacent 45.6 MB `.rev` fell from 8,405 chunks to 8 chunks. The packet still does not claim production storage posture because honey traversal/lifecycle were disabled, the endpoint was plaintext tailnet HTTP, socket highwater reached 11 while upload concurrency was 8, `.idx` still reaches 4,599 chunks, and generated AMD header files reached 2,986/2,121 chunks under the default profile. | `task lazy:home-canary-linux-xr-storage-posture`, `scripts/home-canary-linux-xr-storage-posture.sh`, `scripts/test-home-canary-linux-xr-storage-posture.sh`, `docs/release/evidence/home-canary-linux-xr-storage-posture-20260514T021513Z/storage-posture-live-observations.md`, `docs/release/evidence/home-canary-linux-xr-storage-posture-20260514T021513Z/push-storage-summary.env`, prior packet `docs/release/evidence/home-canary-linux-xr-storage-posture-20260513T220442Z/storage-posture-live-observations.md`, older blocker packets `docs/release/evidence/home-canary-linux-xr-storage-posture-20260513T174944Z/storage-posture-live-observations.md` and `docs/release/evidence/home-canary-linux-xr-storage-posture-20260512T034347Z/storage-posture-live-observations.md`, `docs/BENCHMARKS.md`, storage rows in `docs/ops/lazy-traversal-qa-permutation-matrix-2026-05-09.md`, `crates/tcfs-chunks/src/fastcdc.rs`, and `crates/tcfs-sync/src/engine.rs`. | Keep the `.pack`/`.rev` profile fixes as accepted. Next, decide generated-large-file policy, reduce or justify `.idx` and generated-header chunk counts, fix socket-pool accounting, and rerun with honey traversal/lifecycle plus endpoint/TLS posture before upgrading the storage claim. Multipart/native SeaweedFS writes and raw `.git` defaults remain future decisions if object counts remain too high. |
