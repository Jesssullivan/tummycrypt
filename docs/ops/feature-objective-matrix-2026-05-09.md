...
| S3-first storage posture | Prove the S3/SeaweedFS object model, chunk sizing, retry behavior, per-chunk timeout behavior, queue/concurrency posture, and endpoint/TLS/credential class are acceptable for project-tree workloads. | Open/blocking. `home-canary-linux-xr-shadow-20260511T040325Z/` remains functional project-tree proof, not storage posture. `home-canary-linux-xr-storage-posture-20260512T034347Z/` completed the 6.2 GB pack but exposed no-progress gaps and slow small-file behavior. PR #367 added opt-in fresh-prefix file upload concurrency, and `home-canary-linux-xr-storage-posture-20260513T174944Z/` proves that `file_upload_concurrency=8`, timeout rows, transport retry rows, socket samples, and fresh-prefix skip telemetry are now visible; it still failed the storage bar because the pack reached only 853 / 70,856 chunks before termination and the endpoint is plaintext tailnet SeaweedFS. Current code now makes the first object-model decision: `.pack` / `.iso` / `.img` use a large sequential FastCDC profile, while `.idx` stays on the moderate pack-index profile. | `task lazy:home-canary-linux-xr-storage-posture`, `scripts/home-canary-linux-xr-storage-posture.sh`, `scripts/test-home-canary-linux-xr-storage-posture.sh`, `docs/release/evidence/home-canary-linux-xr-shadow-20260510T201809Z/storage-posture-observations.md`, `docs/release/evidence/home-canary-linux-xr-shadow-20260511T040325Z/push-storage-summary.env`, `docs/release/evidence/home-canary-linux-xr-storage-posture-20260512T034347Z/storage-posture-live-observations.md`, `docs/release/evidence/home-canary-linux-xr-storage-posture-20260512T125747Z/storage-posture-live-observations.md`, `docs/release/evidence/home-canary-linux-xr-storage-posture-20260513T174944Z/storage-posture-live-observations.md`, `docs/BENCHMARKS.md`, storage rows in `docs/ops/lazy-traversal-qa-permutation-matrix-2026-05-09.md`, `crates/tcfs-chunks/src/fastcdc.rs`, and `crates/tcfs-sync/src/engine.rs`. | Rerun the release-binary large raw-Git storage-posture packet on a fresh prefix and require the evidence to record the large sequential chunk profile, reduced pack object count, retry/timeout rows, endpoint/TLS posture, and honey hydration latency before upgrading the storage claim. Multipart/native SeaweedFS writes and raw `.git` defaults remain future decisions if this profile is not enough. |
