...
| S3-first storage posture | Prove the S3/SeaweedFS object model, chunk sizing, retry behavior, per-chunk timeout behavior, queue/concurrency posture, and endpoint/TLS/credential class are acceptable for project-tree workloads. | Open, with push-only progress. `home-canary-linux-xr-shadow-20260511T040325Z/` remains functional project-tree proof, not storage posture. `home-canary-linux-xr-storage-posture-20260513T220442Z/` completed a release-binary fresh-prefix push of the 7.7 GB shadow; the 6.2 GB raw Git `.pack` fell from 70,856 chunks to 1,211 chunks after the large sequential profile. The packet still does not claim production storage posture because honey traversal/lifecycle were disabled, the endpoint was plaintext tailnet HTTP, socket highwater reached 11 while upload concurrency was 8, and the adjacent `.rev` file still leaked 8,405 chunks before the follow-up `.rev` profile fix. | `task lazy:home-canary-linux-xr-storage-posture`, `scripts/home-canary-linux-xr-storage-posture.sh`, `scripts/test-home-canary-linux-xr-storage-posture.sh`, `docs/release/evidence/home-canary-linux-xr-storage-posture-20260513T220442Z/storage-posture-live-observations.md`, `docs/release/evidence/home-canary-linux-xr-storage-posture-20260513T220442Z/push-storage-summary.env`, older blocker packets `docs/release/evidence/home-canary-linux-xr-storage-posture-20260513T174944Z/storage-posture-live-observations.md` and `docs/release/evidence/home-canary-linux-xr-storage-posture-20260512T034347Z/storage-posture-live-observations.md`, `docs/BENCHMARKS.md`, storage rows in `docs/ops/lazy-traversal-qa-permutation-matrix-2026-05-09.md`, `crates/tcfs-chunks/src/fastcdc.rs`, and `crates/tcfs-sync/src/engine.rs`. | Rerun the release-binary storage-posture packet after the `.rev` profile fix and require honey traversal/lifecycle, reduced pack/rev object counts, retry/timeout rows, endpoint/TLS posture, and socket-pool accounting before upgrading the storage claim. Multipart/native SeaweedFS writes and raw `.git` defaults remain future decisions if this profile is not enough. |
