...
| S3 storage posture | PR `#367` landed opt-in fresh-prefix file upload concurrency plus timeout/retry telemetry. The current `home-canary-linux-xr-storage-posture-20260514T021513Z/` packet completes the 7.7 GB shadow push and proves `.pack` / `.rev` object-count reductions, but it is still push-only: endpoint is tailnet HTTP SeaweedFS, socket highwater exceeds configured upload concurrency, `.idx` and generated large headers remain measured object-count follow-ups, and honey lifecycle was disabled. | Production S3 posture, broad `~/git`/home-directory management, or a claim that more client concurrency alone solves large project-tree storage. |
