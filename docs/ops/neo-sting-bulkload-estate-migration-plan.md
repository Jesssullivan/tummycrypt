...
Capture tolerates drift under a pass (R25, bulkload #34: the host need not halt agent work or git work, without ever rewalking or reading a bit twice). Refs appearing, moving or disappearing in the shared ref store (a sibling worktree lane creating branches) and worktree seats appearing, changing or vanishing between the census and their byte pass (`cargo` rewriting `.rustc_info.json`) are recorded, not refused: the item reports `outcome=captured-with-drift` with `reason=drift=N`, the `N` rows (`RefAdded`/`RefChanged`/`RefRemoved`/`SeatAdded`/`SeatChanged`/`SeatRemoved`, sorted, deduplicated, capped at 65,536 beyond which the pass refuses `BUDGET_EXCEEDED`) ride in the corpus `{bundle}.drift` sidecar beside `{bundle}.base` and in the bundle's `refs/carry-export/capture-drift-v1`, and the `Capture` postcard gains no field so every retained `*.capture` record and every journal under `/srv/fast-local/jess/state/git-carry/neo/` still decodes. A drifted seat's bytes are carried by neither the worktree tree nor `filesystem-v1` (R-N28): the drift list is the statement that they are absent, and the next `estate-capture` is an incremental pass that re-reads exactly those seats and emits every seat whose stat identity is unchanged by object name from the retained bundle, reporting `capture-extended-from-drift` and `source_bytes_read` equal to the drifted seats' sizes; `capture-reused-after-census` is unchanged for the clean hit. `estate-apply` of a drifted bundle proceeds, carries the drift rows in its receipt, and when the destination already holds a seat the drift list says was never captured it names that seat as `HeldUncaptured` and never writes over it (R-N29). HEAD moving, the index being rewritten, configuration, the shallow frontier, nested-worktree custody or the omitted rebuildable set changing under a pass still refuse `GIT_AUTHORITY_CHANGED` (R-N30): a lane that commits inside the captured worktree still refuses; a lane in a sibling worktree no longer does. `shared::export_base` and `git-export` keep the strict pre-drift contract.
