...
| macOS | experimental but real; current packages prove package/signing/install paths, earlier hosted lanes proved storage/daemon gates, and PZM proves non-production lab FileProvider enumeration/hydration/evict/rehydrate plus mutation upload/readback and CLI conflict/exact-content preservation under Apple's testing-mode entitlement plus a managed SystemPolicyRule profile; the latest hosted production `.pkg` attempt failed before daemon/Finder on an expired public storage endpoint, and the latest neo local cleanup packets confirm the canonical `/Applications/TCFSProvider.app` install is absent while the registered `~/Applications/TCFSProvider.app` fails strict production preflight because host and extension lack Keychain access-group entitlements and embedded provisioning profiles. A source-built Developer ID app now passes strict signing-only preflight with the compatible local profiles embedded, a local candidate `.pkg` structure/signature proof wraps that app with current source-built `tcfs`/`tcfsd`, and remote run `25973109986` proves the package can be submitted to Apple notarization, accepted, stapled, Gatekeeper install-assessed, and strict-smoked with signature/Gatekeeper/stapled-ticket checks required. Neo then downloaded that notarized artifact, verified SHA-256 `c6fd1a6fd18638c53f0d0b88bc79249e65d08766d99853bef6896ee69bcd6d45`, and reran strict package smoke locally. The follow-up real install attempt is blocked at admin authentication (`sudo -n installer` requires a password), so the package is still not installed locally, not registered through PlugInKit cleanup, and not exercised through Finder. Production Finder enablement/conflict/status UX are still not release-grade | build + packaging + PZM smoke + local desktop evidence + remote/local notarized package proof |
...
2. **Apple desktop acceptance**
   ...
   - latest neo local truth: `/Applications/TCFSProvider.app` is absent; the
     visible registration is under `~/Applications`; strict preflight fails on
     missing Keychain access-group entitlements and embedded provisioning
     profiles; compatible local Developer ID profiles exist but have not been
     embedded into the installed app
...
As of May 16, 2026, the narrow GitHub backlog snapshot is below. Verify live
...
- M10 release-proof tranche
  - `#280`: distribution install and upgrade proof umbrella. Homebrew/Nix
    ...
    `macos-fileprovider-pkg-notarization-proof-20260516T211425Z/`, but
    production macOS `.pkg` release install/Finder proof remains a named
    follow-up.
  - `#309`: macOS `.pkg` clean-host and FileProvider acceptance lane. The
    ...
    package acceptance for a workflow artifact; production Finder lifecycle
    proof remains open.
