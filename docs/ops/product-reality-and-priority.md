...
binaries on both hosts. Rerun `task lazy:git-repo-canary` for Homebrew only
after a rebuilt Homebrew lane is selected; otherwise the next mobility proof is
`~/git/linux-xr-fast` as a larger clean stress canary plus fresh-tree
