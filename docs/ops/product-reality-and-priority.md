...
| macOS | experimental but real; `v0.12.13-rc1` ships a notarized production Developer ID `.pkg`, PZM run `26062554542` proves installed strict preflight, storage `[ok]`, domain add, CloudStorage enumeration, exact hydrate, evict/rehydrate, mutation upload/readback, and conflict-status preservation without `fileprovider_testing_mode=true`, and post-cut run `26079830341` proves the exact published release `.pkg` on the PR #389 branch. Remaining macOS production gaps are first-run setup from installer to valid config/status, badge/progress assertions, recovery UX, and continuous release-day viability | build + packaging + PZM production Dev ID smoke + v0.12.13 evidence matrix + local desktop evidence |
