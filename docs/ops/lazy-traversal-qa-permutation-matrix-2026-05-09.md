...
| --- | --- | --- |
| S6 | Post-fix release-binary raw-Git canary on a new disposable S3 prefix | `20260514T021513Z` archives release build provenance, endpoint/TLS/credential posture, `chunk_exists_check=false`, fresh-prefix publish mode, remote conflict-check skip, `chunk_write_timeout_secs=300`, `file_upload_concurrency=8`, socket samples, heartbeat/progress rows, and completed `.pack`/`.rev` object-count reductions. It is complete as push-only storage evidence, not production storage posture. |
| P6 | Release-binary `linux-xr` storage rerun | Functional mounted symlink closure is green in `20260511T040325Z`. The release-binary storage rerun is push-only green for `.pack`/`.rev`; the next large-tree storage packet should add honey traversal/lifecycle, a production-like TLS endpoint, generated-large-file policy, and socket accounting closure while keeping correctness success separate from production storage posture. |
