...
| M8-A | Delete/rename while peer-unsynced tombstone details | Helper support now records stale `.tc` `sync-status`, repeated old-path pull failure, and repeated new-path hydrate success. Lower-level regressions now cover mounted old-path behavior and delete-then-recreate same relative path. Remaining rows are host packet coverage and rename same-hash versus different-hash cases. Keep product tombstone semantics open until accepted. |
