...
| --- | --- | --- |
| S6 | Post-fix release-binary raw-Git canary on a new disposable S3 prefix | `20260514T021513Z` archives release build provenance, endpoint/TLS/credential posture, `chunk_exists_check=false`, fresh-prefix publish mode, remote conflict-check skip, `chunk_write_timeout_secs=300`, `file_upload_concurrency=8`, socket samples, heartbeat/progress rows, completed `.pack`/`.rev` object-count reductions, and a mounted honey follow-up with `find -maxdepth 8`, exact `.clang-format` hydration, and all 85 mounted symlink target checks. `home-canary-linux-xr-storage-posture-tc-extfix-20260514T202343Z` closes the mounted S3 `NoSuchKey` warning row by dropping 274 warnings to 0 after exact `.tc` filenames stopped being stripped as stub aliases. It is storage evidence with mounted traversal, not production storage posture. |
| P6 | Release-binary `linux-xr` storage rerun | Functional mounted symlink closure is green in `20260511T040325Z`, and the release-binary storage rerun now has same-prefix honey mounted traversal/symlink verification plus mounted warning closure for `.pack`/`.rev` posture. The next large-tree storage packet should add the Linux lifecycle companion, a production-like TLS endpoint, generated-large-file policy, and socket accounting closure while keeping correctness success separate from production storage posture. |
