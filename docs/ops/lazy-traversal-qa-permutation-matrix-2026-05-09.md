...
| --- | --- | --- |
| S6 | Post-fix release-binary raw-Git canary on a new disposable S3 prefix | Archive release build provenance, endpoint/TLS/credential posture, object counts, retries split by transport/timeout, memory peak, wall-clock timings, `chunk_exists_check` mode, fresh-prefix manifest/index publish mode, remote conflict-check mode, `chunk_write_timeout_secs`, `file_upload_concurrency`, S3 HTTP client controls, socket high-water samples, heartbeat/progress rows, and hydration latency. `task lazy:home-canary-linux-xr-storage-posture` is the harness. Debug-binary evidence remains functional evidence only, and mounted traversal/unsync rows are bounded only when their companion configs/logs prove the same S3 limits. |
