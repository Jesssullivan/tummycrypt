...
| S2 | Large raw-Git pack push | Multi-GB `.pack` files push without unbounded memory growth, with archived chunk count, bytes uploaded, retry count, and wall-clock duration | `20260511T040325Z` records the dominant pack shape: 2 pack rows, 70,857 pack chunks, 6,216,112,937 pack bytes, and max object `pack-cca8376c...pack` at 70,856 chunks / 6,216,046,897 bytes. `home-canary-linux-xr-storage-posture-20260512T034347Z` completed the 6.2 GB pack but captured multi-minute no-progress/no-retry gaps. `home-canary-linux-xr-storage-posture-20260513T174944Z` reran after file-concurrency landed; it captured timeout/transport retries and reached only 853 / 70,856 pack chunks before termination. Current code now routes `.pack` through a large sequential FastCDC profile to reduce the dominant object count before the next rerun. Production posture remains blocked until that rerun passes. |
...
| --- | --- | --- |
| S6 | Post-fix release-binary raw-Git canary on a new disposable S3 prefix | `20260513T174944Z` archives release build provenance, endpoint/TLS/credential posture, retry split, `chunk_exists_check=false`, fresh-prefix publish mode, remote conflict-check skip, `chunk_write_timeout_secs=300`, `file_upload_concurrency=8`, socket samples, heartbeat/progress rows, and the failure boundary. It is complete as blocker evidence, not a pass. The next acceptance row should rerun with the large sequential `.pack` chunk profile recorded in `storage-posture.env`. |
| P6 | Release-binary `linux-xr` storage rerun | Functional mounted symlink closure is green in `20260511T040325Z`. The remaining large-tree rerun is storage-focused: use release binaries on a new disposable prefix through the storage-posture wrapper, archive fresh telemetry, prove reduced `.pack` object count, and keep correctness success separate from production storage posture. |
