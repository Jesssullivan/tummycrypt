...
| macOS Finder/FileProvider, lab | `v0.12.11` proved testing-mode enumerate/hydrate; `v0.12.12` evict/rehydrate attempt is blocked at extension runtime policy after the extension starts | PZM run `25446601375` green for read/hydrate; run `25451788783` shows AppleSystemPolicy termination of the extension | Rebuild with host stderr diagnostics, capture `spctl`/codesign/profile evidence, then decide Mac App Development vs Ad Hoc lab shape |
...
1. `taskgated-helper` accepts the extension profile for
   `io.tinyland.tcfs.fileprovider`.
2. `fileproviderd` starts the extension process.
3. About two minutes later, AppleSystemPolicy terminates the extension:
   `Security policy would not allow process ... TCFSFileProvider`.
4. The old harness then times out waiting for a host `add: OK` log, so the next
   package must include direct host stderr diagnostics and stricter signing
   evidence capture.
...
| --- | --- | --- | --- |
| A. PZM runtime-policy diagnosis | macOS lab package only | Run PZM smoke with host stderr diagnostics, `spctl`, codesign, embedded profile, `taskgated`, `amfid`, and AppleSystemPolicy logs attached | B, C, D |
