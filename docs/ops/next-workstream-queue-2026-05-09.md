...
| --- | --- | --- | --- | --- |
| Production macOS Finder/FileProvider | [#309](https://github.com/Jesssullivan/tummycrypt/issues/309), [TIN-133](https://linear.app/tinyland/issue/TIN-133/prove-lazy-traversal-and-finderfileprovider-hydration-reality) | PZM testing-mode proof is green through enumerate, hydrate, evict, rehydrate, mutation, and deterministic conflict/status content preservation. Extended fleet lifecycle proof is archived and tracker-linked. | Pick one production Developer ID clean-host executor, then run the published `.pkg` through install, host launch, domain presence, CloudStorage enumeration, exact-content hydrate, and log capture. | PZM testing-mode and isolated fleet proof do not close production Finder acceptance. Finder badge/progress assertions stay observational until reliable. |
...
1. Production Finder executor decision.
   Pick the executor for #309: GitHub-hosted macOS with public storage,
   private-network hosted runner, self-hosted VM, or manual Darwin lane. Record
   the fallback rule before running.
