...
3. Expendable real-project fleet proof.
   ...
   preserved, and `restore-proof-source-fix-symlink-state-20260515T171712Z/`
   proves source-built fresh-tree restore for regular files, symlinks, and
   synced state. Next, rebuild/prove package restore, close the empty-directory
   gap or keep it explicit, then repeat with a `~/git/linux-xr-fast` shadow and
   package-backed fresh-tree restore/rollback proof.
