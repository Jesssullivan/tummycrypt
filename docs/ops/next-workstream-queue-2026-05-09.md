...
3. Expendable real-project fleet proof.
   ...
   lanes are green, but
   `git-repo-canary-oauth-mux-nixpkg-20260515T133843Z/restore-proof/` shows
   fresh-tree restore is blocked by `tcfs reconcile` remote-index dry-run
   timeout. Fix or bypass that restore path, then repeat with a
   `~/git/linux-xr-fast` shadow and green fresh-tree restore/rollback proof.
