...
| S3 storage posture | [#309](https://github.com/Jesssullivan/tummycrypt/issues/309), [#280](https://github.com/Jesssullivan/tummycrypt/issues/280), [TIN-133](https://linear.app/tinyland/issue/TIN-133/prove-lazy-traversal-and-finderfileprovider-hydration-reality) | The scoped `linux-xr` parity packet is functionally green, and the storage observations exposed the raw-Git `.pack`/`.idx` object shape. PR #367 is merged and proves the fresh-prefix file-concurrency knob plus timeout/retry telemetry are real. The current post-merge packet `home-canary-linux-xr-storage-posture-20260513T174944Z/` is still blocker evidence: `file_upload_concurrency=8` was active, but the 6.2 GB pack reached only 853 / 70,856 chunks before the run was stopped, with timeout and transport retry rows captured. The first object-model fix is now local: raw Git `.pack` files use the large sequential FastCDC profile instead of the moderate `.idx` profile. | Rerun the storage-posture helper with a release binary and fresh prefix so the packet proves the new `.pack` chunk profile, reduced object count, retry/timeout behavior, endpoint/TLS posture, and honey hydration latency. Keep multipart/native SeaweedFS writes, batching strategy, and raw `.git` default policy as explicit follow-ups if the rerun remains blocked. | Correctness success and lab S3 observations do not imply production storage readiness. Do not claim production S3 posture from the current tailnet HTTP SeaweedFS packets. |
...
4. S3 storage posture decision.
   The file-concurrency packet is archived and still blocks production posture.
   Stop increasing client-side concurrency until the object-model decision is
   made: multipart/native SeaweedFS writes, large-tree batching, TLS endpoint
