...
| Fleet/home-directory parity | [#309](https://github.com/Jesssullivan/tummycrypt/issues/309), [TIN-133](https://linear.app/tinyland/issue/TIN-133/prove-lazy-traversal-and-finderfileprovider-hydration-reality) | `docs/release/evidence/fleet-pilot-extended-20260509T2152Z/` proves isolated `Documents`/`git` traversal, hydration, mounted write/readback, cache clear/rehydrate, recursive safe-unsync refusal/success, and live `neo-honey` smoke. `docs/release/evidence/git-repo-canary-oauth-mux-sourcebin-fresh-20260515T014640Z/` now proves a clean `~/git/oauth-mux` shadow with source-built binaries on both hosts: fresh-prefix push, 0 skipped symlinks, honey mounted traversal/hydration, 9 mounted symlink target checks, and Linux lifecycle. Current Nix flake packages pass the tiny symlink mounted parse/target proof on neo and honey (`tcfs-symlink-package-probe-20260515T060330Z/`). Homebrew package drift is still real: installed Homebrew `0.12.12` skipped symlinks. | Rebuild/publish the symlink-preserving Homebrew lane or select the current Nix package lane, reprove the same small repo canary without source-built overrides, then repeat with `~/git/linux-xr-fast` as a large clean stress canary and add fresh-tree restore/rollback proof. Keep broad `~/Documents` or `~/git` management out until package-backed real-project lanes are boring. | The archived fleet packet and new canary task use disposable shadows/prefixes, not real home-directory takeover. Symlink status/unsync semantics are not yet a claim; the current symlink gate is remote publication plus mounted target verification. Source-built success does not equal packaged release readiness. |
| S3 storage posture | [#309](https://github.com/Jesssullivan/tummycrypt/issues/309), [#280](https://github.com/Jesssullivan/tummycrypt/issues/280), [TIN-133](https://linear.app/tinyland/issue/TIN-133/prove-lazy-traversal-and-finderfileprovider-hydration-reality) | The scoped `linux-xr` parity packet is functionally green, and the storage observations exposed the raw-Git `.pack`/`.idx` object shape. PR #367 is merged and proves the fresh-prefix file-concurrency knob plus timeout/retry telemetry are real. The current storage packet completes the 7.7 GB shadow with `file_upload_concurrency=8`, proves same-prefix honey mounted traversal/hydration and all 85 mounted symlink targets, keeps the 6.2 GB `.pack` at 1,211 chunks, and reduces the 45.6 MB `.rev` from 8,405 chunks to 8 chunks. The remaining measured storage blockers are missing lifecycle in the storage packet, `.idx` at 4,599 chunks, generated AMD headers at 2,986/2,121 chunks, plaintext tailnet HTTP, and socket highwater 11 vs upload concurrency 8. | Decide generated-large-file policy and socket-pool accounting, then rerun the storage-posture helper with lifecycle and a production-like TLS endpoint before upgrading the claim. Keep multipart/native SeaweedFS writes, batching strategy, and raw `.git` default policy as explicit follow-ups if object counts remain high. | Correctness success and lab S3 observations do not imply production storage readiness. Do not claim production S3 posture from the current tailnet HTTP SeaweedFS packet. |
...
3. Expendable real-project fleet proof.
   ...
   `~/git/oauth-mux` shadow. The source-built binary lane is green in
   `git-repo-canary-oauth-mux-sourcebin-fresh-20260515T014640Z/`; the next
   proof must close packaged binary drift and rerun the same shape without
   source-built overrides. Then repeat with a `~/git/linux-xr-fast` shadow and
   add fresh-tree restore/rollback proof. Keep this outside real `~/git`
   takeover.
...
4. S3 storage posture decision.
   ...
   blocked. Stop increasing
   client-side concurrency until generated-large-file policy, `.idx` posture,
   socket-pool accounting, endpoint/TLS class, and honey lifecycle scope are
   decided. Keep multipart/native SeaweedFS writes, large-tree batching, TLS
   endpoint posture, and raw `.git` defaults as explicit follow-up decisions.
