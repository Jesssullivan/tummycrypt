...
3. Expendable real-project fleet proof.
   ...
   stalls around a 387 MB `.git/objects/pack/*.idx` upload. Source now contains
   the Git pack-index large-profile fix; rerun the large stress lane only after
   the selected candidate binary/package includes it, then add package-backed
   fresh-tree restore/rollback proof.
...
4. S3 storage posture decision.
   ...
   next raw-Git object-count problem; source now treats Git pack indexes as
   large sequential files, but generated-large-file policy, socket-pool
