...
| `home-canary-linux-xr-storage-posture-20260514T021513Z/` | Push-only release-binary/fresh-prefix storage-posture rerun from `main` `c0c2c0c`: completed the 7.7 GB `linux-xr` shadow push and proved the raw Git `.rev` large sequential profile reduced the dominant 45.6 MB reverse index from 8,405 chunks to 8 chunks | repo-archived source/shadow inventory, symlink target manifests, release binary SHA-256, compressed full `push.log.gz`, `push-storage-summary.env` with 92,969 upload rows / 8.23 GB / 327,482 chunks / no retry or error rows, socket samples showing highwater 11 vs concurrency 8, and `storage-posture-live-observations.md`; honey traversal/lifecycle were disabled, the endpoint was plaintext HTTP, `.idx` still reached 4,599 chunks, and generated AMD header files reached 2,986/2,121 chunks, so this is not production S3 posture or full parity evidence |
