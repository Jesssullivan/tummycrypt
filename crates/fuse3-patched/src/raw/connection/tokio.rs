...
struct BlockFuseConnection {
    ...
    read: Mutex<()>,
    write: Mutex<()>,
    /// FUSE-T handle — keeps the NFS loopback server alive while mounted.
    #[cfg(all(target_os = "macos", feature = "fuse-t"))]
    _fuse_t_handle: Option<crate::fuse_t::FuseTHandle>,
}

#[cfg(any(target_os = "linux", target_os = "macos"))]
impl std::fmt::Debug for BlockFuseConnection {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("BlockFuseConnection")
...
impl BlockFuseConnection {
    ...
    async fn new_with_unprivileged(
        ...
        let mount_path_ref = mount_path.as_ref();

        // Try FUSE-T first (NFS loopback, no kernel module needed)
        #[cfg(feature = "fuse-t")]
        {
            match crate::fuse_t::fuse_t_mount(mount_path_ref) {
                Ok((fd, handle)) => {
                    debug!("FUSE-T mount succeeded on {:?}", mount_path_ref);
                    let file = unsafe { File::from_raw_fd(fd) };
                    return Ok(Self {
                        file,
                        read: Mutex::new(()),
                        write: Mutex::new(()),
                        _fuse_t_handle: Some(handle),
                    });
                }
                Err(e) => {
                    debug!("FUSE-T not available, falling back to macFUSE: {}", e);
                }
            }
        }

        // Fall back to macFUSE (original implementation)
        Self::new_with_unprivileged_macfuse(mount_options, mount_path_ref).await
    }

    #[cfg(target_os = "macos")]
    async fn new_with_unprivileged_macfuse(
        mount_options: MountOptions,
        mount_path: &Path,
    ) -> io::Result<Self> {
